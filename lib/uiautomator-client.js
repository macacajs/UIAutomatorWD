...
function UIAutomator(options) {
  ...
  Object.assign(this, {
    ...
    sysPort:  process.env.UIAUTOMATOR_PORT || 9001,
    urlBase: 'wd/hub'
  }, options || {});
}
...
UIAutomator.prototype.init = function *(adb) {
  this.adb = adb;
  this.initProxy();
  this.sysPort = yield detectPort(this.sysPort);
  yield this.adb.forward(this.sysPort, this.proxyPort);
...
UIAutomator.prototype.startServer = function() {
  return new Promise(resolve => {
    let args = `shell am instrument -w -r -e class ${UIAUTOMATORWD.PACKAGE_NAME} ${UIAUTOMATORWD.TEST_PACKAGE}.test/${UIAUTOMATORWD.RUNNER_CLASS}`.split(' ');
