...
UIAutomator.prototype.init = function *(adb, permissionPatterns) {
  ...
  try {
    let pak1 = yield this.adb.shell(`pm list packages | grep ${UIAUTOMATORWD.TEST_PACKAGE}$`); //如果包没安装，会走异常流程
    if (pak1 != '' && (pak1.split(":"))[1] === UIAUTOMATORWD.TEST_PACKAGE) {
  ...
  try {
    let pak2 = yield this.adb.shell(`pm list packages | grep ${UIAUTOMATORWD.TEST_PACKAGE}.test$`); //如果包没安装，会走异常流程
    if (pak2 != '' && (pak2.split(":"))[1] === 'com.macaca.android.testing.test') {
