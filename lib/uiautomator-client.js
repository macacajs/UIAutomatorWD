...
UIAutomator.prototype.init = function *(adb, permissionPatterns) {
  ...
  try {
    let pak1 = yield this.adb.shell(`pm list packages | grep ${UIAUTOMATORWD.TEST_PACKAGE}$`); 
  ...
  // install dirver pkg olny when pkg not exists
  try {
    let pak2 = yield this.adb.shell(`pm list packages | grep ${UIAUTOMATORWD.TEST_PACKAGE}.test$`);
