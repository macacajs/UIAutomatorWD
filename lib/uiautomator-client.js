...
UIAutomator.prototype.startServer = function() {
  return new Promise(resolve => {
    let args = `shell am instrument -w -r -e permissionPattern ${JSON.stringify(this.permissionPatterns)} -e port ${this.proxyPort} -e class ${UIAUTOMATORWD.PACKAGE_NAME} ${UIAUTOMATORWD.TEST_PACKAGE}.test/${UIAUTOMATORWD.RUNNER_CLASS}`.split(' ');
